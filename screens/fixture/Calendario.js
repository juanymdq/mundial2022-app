...
import { getCalendar } from '../../src/actions/TeamsActions';
...
const Calendario = ({ route, navigation}) => {
    ...
    const calendar = useSelector(state => state.teams.calendar)

    const dispatch = useDispatch()

    useEffect(() => {        
        dispatch(getCalendar(grupo))        
    },[])
  ...
  return (
    <View style={styles.container}> 
        <ImageBackground source={require('../../assets/images/fondo1.jpg')} resizeMode="cover" style={tw.style("flex h-full")}>
            <View style={tw.style(`flex flex-row justify-start items-center border rounded-md h-20 mt-10 bg-${color}-300 ml-2 mr-2`)}>
                <Pressable 
            ...
            <ScrollView>
            {
                calendar.map(obj => {
                    return (                        
                        <View key={obj.id} style={tw.style(`flex flex-col border rounded-md m-2 p-1 mt-5 bg-${color}-300`)}>
                            <View style={tw.style("flex flex-row items-center m-1 p-1")}>
                                <Text style={tw.style("text-xl font-bold w-50 text-left")}>{obj.e1}</Text>
                            ...
                            <View style={tw.style("flex flex-row items-center m-1 p-1")}>
                                <Text style={tw.style("text-xl font-bold w-50 text-left")}>{obj.e2}</Text>
                                <TextInput style={tw.style("border rounded-md ml-2 text-lg p-2 w-15")}></TextInput>
                            </View>                                
                            <Text style={tw.style("text-lg text-center")}>{obj.fecha}</Text>
                        </View>                        
                    )                   
                })                
            }
